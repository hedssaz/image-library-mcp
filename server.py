        ...
        for redirect in range(6):
            async with session.get(url, allow_redirects=False) as response:
                if response.status in {301, 302, 303, 307, 308}:
                    ...
                    url = download_url(str(url.join(URL(location))))
                    continue
...
class ImageLibrary:
    def __init__(self, data_dir: Path, public_base_url: str):
    ...
    def line(self, row) -> str:
        ...
        detail = " / ".join(labels).replace("|", "\\|") or "无别名/描述"
    ...
    def search(self, query: str, limit: int, offset: int) -> str:
        ...
        where = " AND ".join(
            "instr(fold(name || ' ' || aliases || ' ' || description), ?) > 0" for _ in terms
        ) or "1"
        with self.connect() as db:
            rows = db.execute(
                f"""SELECT filename, name, aliases, description FROM images WHERE {where}
                ORDER BY (fold(name) = ?) DESC, rowid DESC LIMIT ? OFFSET ?""",
                [*terms, fold(clean(query)), limit, offset],
            ).fetchall()
        return "\n".join(self.line(row) for row in rows) or "没有找到匹配的图片。"
    ...
    def addalias(self, name: str, aliases: list[str]) -> str:
        with self.connect() as db:
            ...
            row = dict(row)
            row["aliases"] = json.dumps(
                aliases_unique(json.loads(row["aliases"]) + aliases), ensure_ascii=False
            )
    ...
    def get(self, name: str) -> CallToolResult:
        ...
        return CallToolResult(
            content=[TextContent(type="text", text=self.line(row)),
                     ImageContent(type="image", mimeType=row["mime"],
    ...
    def show_image(self, name: str) -> CallToolResult:
        with self.connect() as db:
            row = db.execute(
                "SELECT filename, name, aliases, description, data FROM images WHERE name = ?", (name,)
            ).fetchone()
        if row is None:
            raise ValueError("图片不存在，请先 search 查询准确的图片名字。")
        return CallToolResult(
...
class MCPGuard:
    ...
    def __init__(self, app, token: str, path: str):
        self.app = app
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not hmac.compare_digest(token.encode(), self.token):
            return await JSONResponse({"error": "需要有效的 Bearer token。"}, 401,
        ...
        while True:
            ...
            if len(data) > MAX_REQUEST_BYTES:
                return await JSONResponse({"error": "请求过大。"}, 413)(scope, receive, send)
...
def build_app(mcp_url: str, public_base_url: str, token: str, data_dir: Path):
    ...
    if (endpoint.scheme not in {"http", "https"} or not endpoint.host
            or endpoint.path in {"", "/"} or endpoint.path.endswith("/")
            or endpoint.query_string or endpoint.fragment or endpoint.user is not None):
        raise ValueError("MCP_URL 必须是完整接口地址，例如 https://example.com/mcp。")
    image_origin = URL(public_base_url)
    if (image_origin.scheme not in {"http", "https"} or not image_origin.host
            or image_origin.path not in {"", "/"} or image_origin.query_string or image_origin.fragment
    ...
    async def search(query: Annotated[str, Field(max_length=200)] = "",
        ...
        """
        return text_result(await asyncio.to_thread(library.search, query, limit, offset))
    ...
    async def add(name: Name, url: Annotated[str, Field(max_length=8192)] | None = None,
        ...
        data = await download_image(url) if url is not None else await asyncio.to_thread(decode_image, base64_data)
        return text_result(await asyncio.to_thread(library.add, name, labels, clean(description), data))
    ...
    async def addalias(name: Name, aliases: Annotated[Aliases, Field(min_length=1)]) -> Annotated[CallToolResult, TextOutput]:
        ...
        """
        return text_result(await asyncio.to_thread(library.addalias, clean(name), aliases_unique(aliases)))
    ...
    async def delete(name: Name) -> Annotated[CallToolResult, TextOutput]:
        ...
        """
        return text_result(await asyncio.to_thread(library.delete, clean(name)))

    @mcp.custom_route("/images/{filename}", methods=["GET", "HEAD"])
    async def image_route(request):
        row = await asyncio.to_thread(library.get_image, request.path_params["filename"])
        headers = {"Cache-Control": "no-store", "X-Content-Type-Options": "nosniff"}
        if row is None:
            return Response(status_code=404, headers=headers)
        headers["Cache-Control"] = "public, max-age=86400"
        headers["ETag"] = '"' + request.path_params["filename"] + '"'
        if request.headers.get("if-none-match") == headers["ETag"]:
            return Response(status_code=304, headers=headers)
        headers["Content-Length"] = str(len(row["data"]))
        return Response(b"" if request.method == "HEAD" else row["data"],
...
def create_app():
    ...
    return build_app(mcp_url, public_base_url, token, Path(os.environ.get("DATA_DIR", "./data")))
